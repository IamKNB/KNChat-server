...
def decode_cursor(cursor: str | None) -> KeysetAnchor | None:
    ...
    return KeysetAnchor(sort_time=sort_time, conversation_id=conversation_id)
...
def build_keyset_filter(*, anchor: KeysetAnchor) -> ColumnElement[bool]:
    return or_(
        col(Conversation.created_at) < anchor.sort_time,
        and_(
            col(Conversation.created_at) == anchor.sort_time,
            col(Conversation.id) < anchor.conversation_id,
        ),
    )
